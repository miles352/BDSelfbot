...
const Selfbot = (() => {
  ...
  /*},*/
    ...
    "defaultConfig": [{
      "type": "textbox",
      "id": "this.settings.token",
      "name": "this.settings.token",
      "note": "",
      "value": "",
      "placeholder": "this.settings.token goes here..."
  }, {
      "type": "category",
      "id": "dankmemer",
  ...
  return !global.ZeresPluginLibrary ? class {
    ...
    const plugin = (Plugin, Library) => {
      ...
      const SendMessage = BdApi.findModuleByProps("sendMessage").sendMessage;
              ...
              commands: [
                 ...
                 },
                {
                  ...
                  execute: async function(e, t) {
                    try {
                      let messageId;
                      SendMessage(t.channel.id, { content: ":airplane:                                                                         :office:", nonce: Date.now() * (2 * 22), validNonShortcutEmojis: [] }).then(res => messageId = res.body.id)
                      await wait(500);
                ...
                },
                {
                  ...
                  options: [],
                  execute: function(e, t) {
                    SendMessage(t.channel.id, { content: `||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||
          ||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||
          ||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||
          ||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||
          ||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||||     ||
           ...
           ‍`, nonce: Date.now() * (2 * 22) })
                ...
                },
                {
                  ...
                  execute: async function(e, t) {
                      ...
                      try {
                        ...
                        if (args[0].channelId == t.channel.id && args[0].message.author.id == botId && args[0].message.mentions[0].id == myId) {
                          if (content.includes("Where do you want to search")) {
                            ...
                            SendMessage(t.channel.id, { content: thingToSearch, nonce: Date.now() * (2 * 22) });
                          } else if (content.includes("What type of meme do you want to post")) {
                            SendMessage(t.channel.id, { content: "k", nonce: Date.now() * (2 * 22) });
                            ...
                            SendMessage(t.channel.id, { content: dragonWord, nonce: Date.now() * (2 * 22) });
                            ...
                            SendMessage(t.channel.id, { content: fishWord, nonce: Date.now() * (2 * 22) });
                    ...
                    while (true) {
                      SendMessage(t.channel.id, { content: "pls fish", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls hunt", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls beg", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls postmemes", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls search", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls hl", nonce: Date.now() * (2 * 22) });
                      await wait(2000);
                      SendMessage(t.channel.id, { content: ["high", "low", "jackpot"][Math.floor(Math.random() * 3)], nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls trivia", nonce: Date.now() * (2 * 22) });
                      await wait(2000);
                      SendMessage(t.channel.id, { content: ["A", "B", "C", "D"][Math.floor(Math.random() * 4)], nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls gamble 10", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls snakeeyes 10", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls slots 10", nonce: Date.now() * (2 * 22) });
                      await wait(4000);
                      SendMessage(t.channel.id, { content: "pls bj 10", nonce: Date.now() * (2 * 22) });
                      await wait(2000);
                      SendMessage(t.channel.id, { content: "s", nonce: Date.now() * (2 * 22) });
                      await wait(2000);
                      SendMessage(t.channel.id, { content: "pls dep max", nonce: Date.now() * (2 * 22) });
                      await wait(2000);
                    }
                  }
                    }
